...
import app.endpoint_domain.service as endpoint_domain_service
...
def domain_endpoint_get(request, space_id, project_reference, domain_name):
    project = project_service.get_by_reference(space_id, project_reference)
    if project is None:
        return (404, {"message": "Project not found"})
    endpoint_config = endpoint_domain_service.get_by_project_and_domain(space_id, project['_id'], domain_name)
    if endpoint_config is None:
        return (404, {"message": "No endpoint domains found"})
    out = [traverse(endpoint_config["structure"], None, i) for i in range(0, random.randint(1,10))]
    return (200, {"data": out})
